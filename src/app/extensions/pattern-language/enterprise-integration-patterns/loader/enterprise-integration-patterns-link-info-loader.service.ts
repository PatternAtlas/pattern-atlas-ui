...
export class EnterpriseIntegrationPatternsLinkInfoLoaderService extends Loader<LinkData> {
  ...
  loadContentFromStore(uri?: string): Promise<Map<string, LinkData>> {
    return this.selectContentFromStore(uri)
      .then(triples => this.mapTriples(triples, uri));
  }

  private async getGraphs(): Promise<Array<string>> {
