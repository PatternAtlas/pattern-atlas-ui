...
export class AddToViewComponent implements OnInit {
    ...
    private _transformer = (node: AddItemNode, level: number) => {
        return {
            expandable: (this.patternmode && !!node.patterns && node.patterns.length > 0) || (!this.patternmode && !!node.patterns && node.patterns.length > 0),
            name: node.name,
            level: level,
            id: node.id,
            uri: node.uri
    ...
    constructor(@Inject(MAT_DIALOG_DATA) public data: { patternlanguages: PatternLanguage[], patternmode: boolean }) {
